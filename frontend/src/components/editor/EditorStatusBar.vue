...
const props = defineProps<{ wordCount: number; selectedWordCount?: number; aiConfigured: boolean; contentChanged: boolean; extractLoading?: boolean; novelTitle: string }>()
...
<template>
  <div style="border-top: 1px solid #eee; padding: 6px 16px; display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
    ...
    <n-text v-if="selectedWordCount" depth="2" style="font-size: 12px; margin-left: 8px; background: #2080f018; padding: 0 6px; border-radius: 4px;">已选 {{ selectedWordCount }} 字</n-text>
    ...
    <template v-if="aiConfigured">
      ...
      <n-button size="small"
        :type="extractLoading ? 'error' : 'default'"
        :secondary="!extractLoading"
        @click="extractLoading ? emit('cancelExtract') : emit('extract')">
        <template #icon><n-icon size="16"><ExtractIcon/></n-icon></template>{{ extractLoading ? '取消提取' : '提取信息' }}
      </n-button>
    </template>
    <template v-else>
